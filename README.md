...
`tools/list` at `https://gateway.pipeworx.io/ai-incident-db/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_incidents \
  -H 'Content-Type: application/json' \
  -d '{"query":"facial recognition bias"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_incidents`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
